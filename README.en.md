...
Prudentia runs Codex CLI as an ephemeral, isolated process with provider-native tools disabled. External research executes through the application's `WebResearchProvider` and Capability Registry; the CLI cannot inspect the workspace, open a browser, or use provider-native multi-agent. On an agent turn the CLI can only decide to call one registered read-only tool or submit the final structured result. The application still owns tool validation, execution, timeout, and persistence. The runtime reports research-cache lookup, retrieval, verification, skill/agent analysis, the agent's current read-only tool, AI reading/analysis/writing, and confirmation-card extraction as persisted stages. Company work also shows a complete research plan independent of internal agent turns: scope, default-template dimensions, total progress, and every step status. Backend events persist that plan so refresh and reconnect never fall back to client inference. The CLI has no real token deltas and still carries Codex agent startup overhead; use the OpenAI-compatible provider for lower time-to-first-token and true streaming. Visible-response hard limits are 90, 240, and 600 seconds for light, standard, and deep tasks respectively. Confirmation-card extraction allows 120 seconds after light/standard turns and 300 seconds after deep turns. Normal model completion and user cancellation still end the task immediately.

Application tools, skills, and agents are owned by one Capability Registry rather than being freely invoked by the CLI. `research_company` and `research_community_insights` are native read-only tools. `analyze_business_model` and `audit_moat` are versioned skills that can run directly or be preloaded into an agent. `analyze_company` and `challenge_company_thesis` are agents with exact skill dependencies, a read-only tool allowlist, and an eight-turn ceiling. A deterministic planner runs base research first, then gives matching capabilities the same frozen evidence snapshot in parallel before the response model synthesizes the result. An agent may call at most four non-duplicate tools; total observations are capped at 96 KiB and each capability output at 48 KiB. Every exact-version call has a stable `call_id`; agent turn, current tool, completion, and failure events are persisted. The frontend restores active work after refresh and renders structured artifacts plus a bounded trace containing only turn, tool, status, and source-count metadata, without duplicating research bodies. Custom JSON manifests load from `capabilities/` under the shared local root, so all worktrees use one personal capability configuration; the directory is Git-ignored by default. `GET /api/conversation/capabilities` also reports an agent's loaded skills and allowed tools. Company-view, trade, and investment-rule writes still require item-level confirmation before deterministic domain code executes them. See [Conversation Capabilities](docs/capabilities.en.md) for the manifest contract, extension boundary, and rule-graph reuse.

Built-in analysis no longer shares one generic prompt. Business-model, moat, comprehensive-company, and dissent work each has a dedicated schema. Artifacts expose evidence sufficiency, fact/inference/hypothesis, claim-level sources and dates, causal chain, strongest counterargument, leading indicators, falsification, and impact on the company view. Insufficient evidence returns a concise abstention, decisive gaps, and at most two hypotheses instead of a category-complete pseudo-analysis. The backend rejects a fact without a real source from that turn and any URL not supplied to the model. Community views remain leads to verify rather than evidence.

Capability form and workflow stage are independent: the deep-company agent runs in `analysis`, while the dissent agent runs in `challenge`. Skills preloaded by a lead agent do not execute again unless the user explicitly requests `@skill_id`, and a repeated research URL is persisted only once within one run.

Model capabilities declaring `rule_graph` must also grant `rule_graph_input` explicitly. Graphs pin the capability version and manifest content hash and bound nodes, model calls, total runtime, and retained executions. Active-run APIs return unfinished capability snapshots, long event backlogs replay completely, and APIs/UI expose only stable safe failure categories rather than provider/CLI internals.
...
Company threads currently analyze the enterprise only. Home opens the latest active thread after reload. Confirmed company views are stored as local versions and shown by default in the Company View tab of every thread bound to that company. When a bound company thread explicitly asks about another held company, the thread binding stays unchanged while research, sources, company context, and turn summaries switch to the explicitly named company; the bound company's summaries, view, and research cache are not reused. A company short name resolves automatically only when holdings, the bound company, a security code, and the local symbol directory produce one high-confidence candidate. A unique English typo within the bounded edit distance, such as `netlfix`, also resolves automatically; tied candidates remain a clarification. A lowercase pure-letter ticker must also sit next to explicit investment, analysis, research, or company-question wording, preventing greeting words from becoming securities. Multiple or unknown candidates trigger a request for the full company name or security code; no research runs and the thread company is never used as a fallback before confirmation. The original request and candidates are persisted with the clarification message. A security code, an ordinal such as “the first one,” or an explicit affirmation after a single-candidate question resumes the request and automatic research. A recently canceled AI confirmation question is recoverable only when it uniquely names one company in the local symbol directory. A new complete request supersedes that clarification; a still-ambiguous selection asks again. Once identity resolves, scope selection does not block on another question. An unrestricted request uses the comprehensive template covering business model, owner economics, competitive position, moat, management and capital allocation, financial resilience, earning power, and failure mechanisms; an explicit focused request displays that scope and starts directly. Context sent to the response model excludes portfolio summaries, positions, personal trade history, and the investment system. Share prices, quotes, market capitalization, valuation multiples, price targets, stock returns, technical analysis, ratings, and personal P/L are ignored even when they occur in prior messages or sources. Product markets, industry structure, competitors, and market share remain in scope. Replies end with company quality and operating uncertainty, without valuation, a security view, portfolio impact, or trading conclusions. Automatic company-view patches leave `valuation_expectations` unset, and `thesis` means the operating company thesis only.
