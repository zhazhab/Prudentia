...
pub async fn update_position(
    ...
    record_position_baseline(pool, &position, "position_update", &position.updated_at).await?;
...
pub async fn delete_position(
    ...
    let deleted = get_position(pool, &normalized).await?;
    ...
    recompute_weights_with_fx(pool, market_data.clone()).await?;
    record_zero_position_baseline(pool, &deleted, "position_delete").await?;
