...
Draft `market` and `currency` fields remain editable. Each market displays native market value in its own currency, while the fixed base currency `CNY` is used for total portfolio value and weights. The frontend displays money with an ISO currency-code prefix, such as `CNY 1,234.56`, `HKD 1,234.56`, or `USD 1,234.56`; the holdings table can be sorted by market value, P/L, return rate, period return rate, and weight, and the latest sort rule is saved locally in the browser.
...
Import confirmation, draft confirmation, position edits, and position deletes all record portfolio performance snapshots and current position snapshots; when these actions change the CNY portfolio value, the system automatically records a `buy` or `sell` trade adjustment. Daily quote refreshes only write snapshots and never create trade adjustments; benchmark snapshots are written only during the holding price-refresh cycle. The Portfolio Performance `this month`, `this year`, and `since inception` views read trade adjustments in the same period and calculate portfolio return as time-weighted return: each snapshot interval uses `(ending value - interval net trade adjustment) / starting value - 1`, then the interval returns are compounded; amount P/L is `end_value_base - start_value_base - net_cash_flow_base`; the unadjusted `simple_return_pct` is also returned as an explanation field. Deleting down to an empty portfolio becomes the new starting boundary for later reads, so cleanup followed by reimport is not counted as investment return. Annualized return is based on the time-weighted cumulative return; a single starting snapshot with zero return displays 0. If no snapshot exists at the period boundary, the UI displays the first available snapshot date. The holdings table reuses the same period selector and displays period return from each holding's CNY value snapshot change; current return rate still uses native unrealized P/L divided by cost basis.

Benchmark comparisons use S&P ETF proxy `SPY`, Hang Seng ETF proxy `2800.HK`, and the official SSE Composite `000001.SS`. Fetch failures only mark the benchmark unavailable/stale and do not block portfolio performance reads. The frontend supports cumulative return, annualized return, and relative excess return dimensions and shows the benchmark quote source in status; excess return is displayed as portfolio cumulative return minus the benchmark cumulative return.

## Holding Baselines And Conversation Trades

Every confirmed import or manual holding update establishes a new ledger baseline for that security, with baseline quantity and average cost equal to the current position at that time. A confirmed conversation trade only recomputes the current position when its execution time is later than that baseline. Earlier trades remain immutable history and are explicitly marked as not affecting the current projection. This allows users to tell the system what they hold now without reconstructing all prior deposits, withdrawals, and trades.

Post-baseline buys include fees in cost and use a quantity-weighted average. Sells do not change the remaining average cost, and selling more than the available quantity is rejected. Buys are TWR cash inflows and sells are cash outflows, so no separate deposit/withdrawal record is required. Non-CNY trades require execution-date historical FX: the system first queries the market-data provider and preserves its source; when unavailable, the user must provide it manually, and the current FX rate is never substituted.

Confirmed trade events cannot be overwritten. A correction creates a reversal event plus a replacement trade, then rebuilds position and portfolio-return projections from the baseline. Broker transaction sync remains out of scope, and every conversation trade executes only after its confirmation card is approved.
