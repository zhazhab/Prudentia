...
Every visible reply comes from the configured real provider, including greetings and capability questions. Pure greetings or capability questions without attachments or research results skip structured-action projection after the visible response; investment content retains the complete projection path. OpenAI-compatible providers parse real SSE tokens. Codex CLI continuously consumes JSONL run events; when it has no token delta, only phases update and the final body appears once. `AI_PROVIDER` supports ordered fallback, but can only switch before the previous provider emits body text, and mock is never selected automatically. External research uses a `WebResearchProvider` interface with a keyless public-sources adapter by default, optional Tavily, and a disabled adapter. The default adapter concurrently retrieves recent SEC filings plus their primary filing/earnings attachment, ticker-linked Yahoo Finance news/analysis, and TradingView company ideas with hot/engagement data. Complete and partial results are cached for 24 hours per provider; partial results carry an evidence-class warning. Community sources are labeled `community` and treated only as viewpoint/sentiment signals. Unavailable search continues with local context and explicitly marks external verification incomplete.

Research planning applies only to subjects resolved as `company`. The provider boundary preserves only the company name/symbol and a structured research intent, never raw conversation text, position sizes, or other private context. The public-sources adapter performs exactly three retrievals and Tavily performs exactly three query classes, so the request budget does not depend on model self-restraint. SEC and TradingView sources accept only same-domain HTTPS URLs returned by those platforms, and TradingView ideas additionally require `is_hot` plus nonzero engagement. Yahoo/Tavily URLs are classified by parsed host and verified with redirects disabled against pinned public-DNS results; when a public site blocks direct retrieval, a read-only public page reader may verify content for the original URL. Redirects, non-success responses, soft 404s, private-network targets, and unverified community engagement never enter model context. The model is also told to treat page and attachment content as untrusted evidence data rather than instructions.
