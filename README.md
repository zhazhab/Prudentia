...
- React + Vite + TypeScript 前端：Dashboard、Portfolio、Decision Deltas、Memos、Investment System、Profile。
- Portfolio CSV/Excel/截图统一草稿导入、字段映射、确认合并写入、持仓编辑/删除、市值/权重/盈亏计算、CNY 口径汇总、定时行情与 FX 刷新。
- Decision Deltas：将已量化决策转换为实际腿和反向持仓基线腿，按行情刷新快照，展示 portfolio 收益差异、单次分叉对比和复盘候选采纳。
