import {
  createContext,
  type ReactNode,
  useContext,
  useEffect,
  useMemo,
  useState
} from "react";

export type Locale = "en" | "zh";

export type TranslationKey = keyof typeof translations.en;

interface I18nContextValue {
  locale: Locale;
  languageTag: string;
  setLocale: (locale: Locale) => void;
  t: (key: TranslationKey, values?: Record<string, string | number>) => string;
}

const I18nContext = createContext<I18nContextValue | null>(null);
const storageKey = "prudentia.locale";

export function I18nProvider({ children }: { children: ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(() => initialLocale());

  useEffect(() => {
    document.documentElement.lang = locale === "zh" ? "zh-CN" : "en";
    window.localStorage.setItem(storageKey, locale);
  }, [locale]);

  const value = useMemo<I18nContextValue>(() => {
    const languageTag = locale === "zh" ? "zh-CN" : "en-US";

    return {
      locale,
      languageTag,
      setLocale: setLocaleState,
      t: (key, values) => interpolate(translations[locale][key] ?? translations.en[key], values)
    };
  }, [locale]);

  return <I18nContext.Provider value={value}>{children}</I18nContext.Provider>;
}

export function useI18n() {
  const value = useContext(I18nContext);
  if (!value) {
    throw new Error("useI18n must be used inside I18nProvider");
  }
  return value;
}

function initialLocale(): Locale {
  const stored = window.localStorage.getItem(storageKey);
  if (stored === "zh" || stored === "en") {
    return stored;
  }

  return navigator.language.toLowerCase().startsWith("zh") ? "zh" : "en";
}

function interpolate(template: string, values: Record<string, string | number> = {}) {
  return Object.entries(values).reduce(
    (result, [key, value]) => result.replaceAll(`{${key}}`, String(value)),
    template
  );
}

const translations = {
  en: {
    "app.subtitle": "Investment OS",
    "app.navLabel": "Main navigation",
    "app.languageLabel": "Language",
    "app.langEnglish": "EN",
    "app.langChinese": "中文",
    "nav.dashboard": "Dashboard",
    "nav.portfolio": "Portfolio",
    "nav.decisionDeltas": "Decision deltas",
    "nav.memos": "Memos",
    "nav.research": "Research",
    "nav.system": "System",
    "nav.profile": "Profile",
    "nav.settings": "Settings",

    "common.open": "Open",
    "common.positions": "{count} positions",
    "common.positionsTracked": "{count} positions tracked",
    "common.stale": "stale",
    "common.fresh": "fresh",
    "common.cancel": "Cancel",

    "dashboard.eyebrow": "Local-first investment workspace",
    "dashboard.title": "Decision cockpit",
    "dashboard.newMemo": "New memo",
    "dashboard.portfolioValue": "Portfolio value",
    "dashboard.unrealizedPl": "Unrealized P/L",
    "dashboard.costBasis": "Cost basis {value}",
    "dashboard.profileLevel": "Profile level",
    "dashboard.level": "Level {level}",
    "dashboard.xpEarned": "{xp} XP earned",
    "dashboard.priceFreshness": "Price freshness",
    "dashboard.staleCount": "{count} stale",
    "dashboard.refreshFromPortfolio": "Refresh prices from Portfolio",
    "dashboard.portfolioWeight": "Portfolio weight",
    "dashboard.noPortfolioTitle": "No portfolio yet",
    "dashboard.noPortfolioBody": "Import a CSV or Excel export to turn holdings into a visible decision surface.",
    "dashboard.recentMemos": "Recent memos",
    "dashboard.noMemosTitle": "No memos yet",
    "dashboard.noMemosBody": "Capture a thesis, risks, catalysts, and kill criteria before decisions harden.",
    "dashboard.profileSignals": "Profile signals",

    "portfolio.eyebrow": "Portfolio",
    "portfolio.title": "Holdings, weight, and price freshness",
    "portfolio.refreshPrices": "Refresh prices",
    "portfolio.marketValue": "Market value",
    "portfolio.costBasis": "Cost basis",
    "portfolio.stalePrices": "Stale prices",
    "portfolio.importPositions": "Import positions",
    "portfolio.chooseFile": "Choose CSV or Excel",
    "portfolio.chooseImage": "Choose screenshot",
    "portfolio.pasteImage": "Paste screenshot",
    "portfolio.recognizeImage": "Screenshot recognition",
    "portfolio.recognizingImage": "Recognizing screenshot...",
    "portfolio.imagePrivacy": "Images are sent to the configured Codex CLI provider for recognition. Recognized rows stay in preview only.",
    "portfolio.imagePreview": "Recognized rows",
    "portfolio.clearImagePreview": "Clear screenshot preview",
    "portfolio.noImagePreviewTitle": "No screenshot recognized",
    "portfolio.noImagePreviewBody": "Upload or paste a screenshot to extract visible holding rows.",
    "portfolio.clipboardUnsupported": "Clipboard image access is not supported in this browser.",
    "portfolio.noClipboardImage": "No image was found on the clipboard.",
    "portfolio.confidence": "Confidence",
    "portfolio.warnings": "Warnings",
    "portfolio.selectColumn": "Select column",
    "portfolio.commitImport": "Commit import",
    "portfolio.noImportTitle": "No import loaded",
    "portfolio.noImportBody": "Upload a broker export, map columns once, and let Prudentia compute value, weight, and P/L.",
    "portfolio.sectorExposure": "Sector exposure",
    "portfolio.noExposureTitle": "No exposure data",
    "portfolio.noExposureBody": "Import positions to see allocation by sector.",
    "portfolio.refreshResult": "Refresh result",
    "portfolio.refreshedCount": "{count} refreshed",
    "portfolio.failedCount": "{count} failed",
    "portfolio.noRefreshTitle": "Prices not refreshed this session",
    "portfolio.noRefreshBody": "The backend also runs a scheduled refresh using the configured market data provider.",
    "portfolio.positions": "Positions",
    "portfolio.tableSymbol": "Symbol",
    "portfolio.tableName": "Name",
    "portfolio.tableQty": "Qty",
    "portfolio.tableAvgCost": "Avg cost",
    "portfolio.tableLastPrice": "Last price",
    "portfolio.tableMarketValue": "Market value",
    "portfolio.tablePl": "P/L",
    "portfolio.tableWeight": "Weight",
    "portfolio.tableStatus": "Status",
    "portfolio.noPositionsTitle": "No positions tracked",
    "portfolio.noPositionsBody": "Import holdings to connect memos, decisions, and portfolio exposure.",
    "portfolio.mapSymbol": "Symbol",
    "portfolio.mapName": "Name",
    "portfolio.mapQuantity": "Quantity",
    "portfolio.mapAverageCost": "Average cost",
    "portfolio.mapCurrency": "Currency",
    "portfolio.mapAccount": "Account",
    "portfolio.mapMarket": "Market",
    "portfolio.mapSector": "Sector",
    "portfolio.mapImportedMarketValue": "Market value",
    "portfolio.mapNotes": "Notes",
    "portfolio.importTools": "Import tools",
    "portfolio.importToolsBody": "Load a file or screenshot into one editable draft table, then confirm the rows you want to merge into holdings.",
    "portfolio.applyMapping": "Apply mapping",
    "portfolio.preparingDraft": "Preparing import draft...",
    "portfolio.clearDraft": "Clear draft",
    "portfolio.commitDraft": "Confirm draft",
    "portfolio.fixDraftErrors": "Fix or remove rows with blocking errors before confirming.",
    "portfolio.emptyDraftTitle": "No draft rows",
    "portfolio.emptyDraftBody": "Choose a CSV, Excel file, or screenshot to prepare editable holdings.",
    "portfolio.rowIssues": "Issues",
    "portfolio.actions": "Actions",
    "portfolio.cnyTotal": "CNY total",
    "portfolio.cnyPl": "CNY P/L",
    "portfolio.marketGroups": "market groups",
    "portfolio.fxStaleCount": "{count} stale FX",
    "portfolio.marketAllocation": "Market allocation",
    "portfolio.fxStale": "FX stale",
    "portfolio.editPosition": "Edit position",
    "portfolio.savePosition": "Save position",

    "decisionDelta.eyebrow": "Decision delta timeline",
    "decisionDelta.title": "Actual decisions versus baseline holdings",
    "decisionDelta.refresh": "Refresh deltas",
    "decisionDelta.refreshing": "Refreshing...",
    "decisionDelta.visibleDelta": "Visible delta",
    "decisionDelta.decisionCount": "Decisions",
    "decisionDelta.staleSnapshots": "Stale snapshots",
    "decisionDelta.lastRefresh": "Last refresh",
    "decisionDelta.never": "Never",
    "decisionDelta.sumDetail": "{positive} positive / {negative} negative",
    "decisionDelta.impactDetail": "Portfolio impact {value}",
    "decisionDelta.filters": "Filters",
    "decisionDelta.symbol": "Symbol",
    "decisionDelta.action": "Action",
    "decisionDelta.delta": "Delta",
    "decisionDelta.stale": "Freshness",
    "decisionDelta.review": "Review",
    "decisionDelta.sort": "Sort",
    "decisionDelta.allSymbols": "All symbols",
    "decisionDelta.allActions": "All actions",
    "decisionDelta.allDeltas": "All deltas",
    "decisionDelta.positive": "Positive",
    "decisionDelta.negative": "Negative",
    "decisionDelta.noSnapshot": "No snapshot",
    "decisionDelta.allFreshness": "All freshness",
    "decisionDelta.staleOnly": "Stale only",
    "decisionDelta.freshOnly": "Fresh only",
    "decisionDelta.allReviews": "All reviews",
    "decisionDelta.reviewedOnly": "Reviewed only",
    "decisionDelta.unreviewedOnly": "Unreviewed only",
    "decisionDelta.sortDate": "Newest decision",
    "decisionDelta.sortAbsoluteDelta": "Largest delta",
    "decisionDelta.sortImpact": "Largest impact",
    "decisionDelta.sortStale": "Stale first",
    "decisionDelta.timeline": "Timeline",
    "decisionDelta.quantified": "quantified",
    "decisionDelta.unquantified": "needs baseline",
    "decisionDelta.reviewed": "reviewed",
    "decisionDelta.notReviewed": "not reviewed",
    "decisionDelta.noTimelineTitle": "No decisions match",
    "decisionDelta.noTimelineBody": "Adjust filters or record a quantified decision.",
    "decisionDelta.noSelectionTitle": "No decision selected",
    "decisionDelta.noSelectionBody": "Select a decision to inspect its actual and baseline legs.",
    "decisionDelta.detail": "Decision detail",
    "decisionDelta.rationale": "Rationale",
    "decisionDelta.expectedOutcome": "Expected outcome",
    "decisionDelta.actual": "Actual",
    "decisionDelta.baseline": "Baseline",
    "decisionDelta.vs": "vs",
    "decisionDelta.assetVsCash": "Holding vs cash",
    "decisionDelta.cashVsHolding": "Cash vs continued holding",
    "decisionDelta.cashVsHypothetical": "Cash kept vs hypothetical buy",
    "decisionDelta.manualComparison": "Actual vs baseline",
    "decisionDelta.latestSnapshot": "Latest snapshot",
    "decisionDelta.actualValue": "Actual value",
    "decisionDelta.baselineValue": "Baseline value",
    "decisionDelta.deltaPct": "Delta %",
    "decisionDelta.snapshotHistory": "Snapshot history",
    "decisionDelta.snapshotHistoryLimited": "Snapshot history · latest {count}",
    "decisionDelta.tableDate": "Date",
    "decisionDelta.tableActual": "Actual",
    "decisionDelta.tableBaseline": "Baseline",
    "decisionDelta.tableDelta": "Delta",
    "decisionDelta.tableStatus": "Status",
    "decisionDelta.fresh": "fresh",
    "decisionDelta.reviewNotes": "Notes",
    "decisionDelta.thesisEvidence": "Thesis evidence",
    "decisionDelta.disconfirmingEvidence": "Disconfirming evidence",
    "decisionDelta.lessons": "Lessons",
    "decisionDelta.candidatePrinciples": "Candidate principles",
    "decisionDelta.candidateChecklist": "Candidate checklist",
    "decisionDelta.saveReview": "Save review",
    "decisionDelta.savingReview": "Saving...",
    "decisionDelta.adoptSelected": "Adopt selected",
    "decisionDelta.adopting": "Adopting...",
    "decisionDelta.saved": "Review saved",
    "decisionDelta.adopted": "Candidates adopted",
    "decisionDelta.noCandidates": "No candidates yet",
    "decisionDelta.refreshResult": "{refreshed} refreshed, {failed} failed",

    "memos.eyebrow": "Memos",
    "memos.title": "Thesis, risks, catalysts, and kill criteria",
    "memos.newMemo": "New memo",
    "memos.fieldTitle": "Title",
    "memos.titlePlaceholder": "Company or idea",
    "memos.fieldSymbol": "Symbol",
    "memos.fieldNotes": "Raw notes",
    "memos.notesPlaceholder": "Paste observations, numbers, links, questions, or a rough thesis.",
    "memos.fieldTags": "Tags",
    "memos.tagsPlaceholder": "quality, compounder",
    "memos.create": "Create memo",
    "memos.library": "Memo library",
    "memos.noMemosTitle": "No memos yet",
    "memos.noMemosBody": "Start with rough notes; the AI provider can extract a structured memo from them.",
    "memos.noSymbol": "No symbol",
    "memos.aiExtract": "AI extract",
    "memos.notes": "Notes",
    "memos.noNotes": "No notes yet",
    "memos.thesis": "Thesis",
    "memos.risks": "Risks",
    "memos.catalysts": "Catalysts",
    "memos.disconfirmingEvidence": "Disconfirming evidence",
    "memos.aiChecklist": "AI checklist",

    "research.eyebrow": "Research library",
    "research.title": "Research records, snapshots, and adoption candidates",
    "research.distill": "Distill source",
    "research.stockSnapshot": "Stock snapshot",
    "research.portfolioReview": "Portfolio review",
    "research.records": "Records",
    "research.filters": "Filters",
    "research.kindAll": "All kinds",
    "research.kindDistillation": "Distillation",
    "research.kindStockSnapshot": "Stock snapshot",
    "research.kindPortfolioReview": "Portfolio review",
    "research.search": "Search",
    "research.symbol": "Symbol",
    "research.sourceType": "Source type",
    "research.sourceTitle": "Source title",
    "research.sourceAuthor": "Source author",
    "research.sourceContent": "Source content",
    "research.recordTitle": "Title",
    "research.run": "Run",
    "research.summary": "Summary",
    "research.insights": "Insights",
    "research.risks": "Risks",
    "research.checklist": "Checklist",
    "research.candidatePrinciples": "Candidate principles",
    "research.candidateChecklist": "Candidate checklist",
    "research.adoptSelected": "Adopt selected",
    "research.loading": "Loading",
    "research.loadingRecords": "Loading research records.",
    "research.loadingSelection": "Loading selected research record.",
    "research.errorTitle": "Research request failed",
    "research.noRecordsTitle": "No research records",
    "research.noRecordsBody": "Run a distillation, stock snapshot, or portfolio review to build the library.",
    "research.noSelectionTitle": "No research selected",
    "research.noSelectionBody": "Select a record to review its summary, risks, checklist, and adoption candidates.",

    "system.eyebrow": "Investment system",
    "system.title": "Principles, checklist, competence, rules",
    "system.aiRefine": "AI refine",
    "system.principles": "Principles",
    "system.checklist": "Checklist",
    "system.circle": "Circle of competence",
    "system.decisionRules": "Decision rules",
    "system.save": "Save system",
    "system.aiRefinement": "AI refinement",

    "profile.eyebrow": "Investor profile",
    "profile.title": "RPG-like feedback from your process",
    "profile.level": "Level",
    "profile.xp": "XP",
    "profile.nextLevel": "Next level at {xp}",
    "profile.badges": "Badges",
    "profile.biasSignals": "Bias signals",
    "profile.attributes": "Attributes",
    "profile.noBadgesTitle": "No badges yet",
    "profile.noBadgesBody": "Create memos, import positions, and record reviewable decisions to unlock feedback.",
    "profile.noSignalsTitle": "No active signals",
    "profile.noSignalsBody": "Current rule checks are not flagging a process risk.",
    "profile.ruleEvents": "Rule events",

    "settings.eyebrow": "Settings",
    "settings.title": "AI provider and credential setup",
    "settings.provider": "Provider",
    "settings.providerMock": "Mock",
    "settings.providerOpenai": "OpenAI-compatible",
    "settings.providerCli": "CLI provider",
    "settings.cliProvider": "CLI provider",
    "settings.cliProviderCodex": "Codex CLI device login",
    "settings.openaiBaseUrl": "OpenAI-compatible base URL",
    "settings.openaiModel": "OpenAI-compatible model",
    "settings.openaiApiKey": "API key",
    "settings.openaiApiKeyPlaceholder": "Leave blank to keep current key",
    "settings.keyConfigured": "A key is currently configured.",
    "settings.keyMissing": "No API key is configured.",
    "settings.cliPath": "CLI path",
    "settings.cliModel": "CLI model override",
    "settings.cliProfile": "CLI profile",
    "settings.cliLoginCommand": "CLI login command",
    "settings.cliHelp": "For Codex, run this in a terminal once, finish the browser/device-code flow, then choose CLI as the provider.",
    "settings.persist": "Write these settings to .env",
    "settings.save": "Save AI settings",
    "settings.saved": "Settings saved",
    "settings.envNote": "Runtime settings take effect immediately. Writing .env makes them survive a backend restart.",
    "settings.mockNote": "Mock mode stays fully local and deterministic.",
  },
  zh: {
    "app.subtitle": "投资操作系统",
    "app.navLabel": "主导航",
    "app.languageLabel": "语言",
    "app.langEnglish": "EN",
    "app.langChinese": "中文",
    "nav.dashboard": "总览",
    "nav.portfolio": "组合",
    "nav.decisionDeltas": "决策差异",
    "nav.memos": "备忘录",
    "nav.research": "研究",
    "nav.system": "投资体系",
    "nav.profile": "画像",
    "nav.settings": "设置",

    "common.open": "打开",
    "common.positions": "{count} 个持仓",
    "common.positionsTracked": "已跟踪 {count} 个持仓",
    "common.stale": "待刷新",
    "common.fresh": "已更新",
    "common.cancel": "取消",

    "dashboard.eyebrow": "本地优先的投资工作台",
    "dashboard.title": "决策驾驶舱",
    "dashboard.newMemo": "新建备忘录",
    "dashboard.portfolioValue": "组合市值",
    "dashboard.unrealizedPl": "未实现盈亏",
    "dashboard.costBasis": "成本基准 {value}",
    "dashboard.profileLevel": "画像等级",
    "dashboard.level": "{level} 级",
    "dashboard.xpEarned": "已获得 {xp} XP",
    "dashboard.priceFreshness": "行情新鲜度",
    "dashboard.staleCount": "{count} 个待刷新",
    "dashboard.refreshFromPortfolio": "在组合页刷新价格",
    "dashboard.portfolioWeight": "组合权重",
    "dashboard.noPortfolioTitle": "还没有组合数据",
    "dashboard.noPortfolioBody": "导入 CSV 或 Excel 持仓文件，把持仓变成可见的决策界面。",
    "dashboard.recentMemos": "最近备忘录",
    "dashboard.noMemosTitle": "还没有备忘录",
    "dashboard.noMemosBody": "在决策定型前，先记录 thesis、风险、催化剂和退出条件。",
    "dashboard.profileSignals": "画像信号",

    "portfolio.eyebrow": "Portfolio",
    "portfolio.title": "持仓、权重与行情新鲜度",
    "portfolio.refreshPrices": "刷新价格",
    "portfolio.marketValue": "市值",
    "portfolio.costBasis": "成本基准",
    "portfolio.stalePrices": "待刷新价格",
    "portfolio.importPositions": "导入持仓",
    "portfolio.chooseFile": "选择 CSV 或 Excel",
    "portfolio.chooseImage": "选择截图",
    "portfolio.pasteImage": "粘贴截图",
    "portfolio.recognizeImage": "截图识别",
    "portfolio.recognizingImage": "正在识别截图...",
    "portfolio.imagePrivacy": "图片会发送给已配置的 Codex CLI provider 进行识别。识别结果只保留在预览中。",
    "portfolio.imagePreview": "识别结果",
    "portfolio.clearImagePreview": "清除截图预览",
    "portfolio.noImagePreviewTitle": "尚未识别截图",
    "portfolio.noImagePreviewBody": "上传或粘贴截图，提取可见的持仓行。",
    "portfolio.clipboardUnsupported": "当前浏览器不支持读取剪贴板图片。",
    "portfolio.noClipboardImage": "剪贴板中没有找到图片。",
    "portfolio.confidence": "置信度",
    "portfolio.warnings": "提示",
    "portfolio.selectColumn": "选择列",
    "portfolio.commitImport": "确认导入",
    "portfolio.noImportTitle": "尚未载入导入文件",
    "portfolio.noImportBody": "上传券商导出文件，完成一次字段映射，Prudentia 会计算市值、权重和盈亏。",
    "portfolio.sectorExposure": "行业暴露",
    "portfolio.noExposureTitle": "暂无暴露数据",
    "portfolio.noExposureBody": "导入持仓后查看行业配置。",
    "portfolio.refreshResult": "刷新结果",
    "portfolio.refreshedCount": "已刷新 {count} 个",
    "portfolio.failedCount": "失败 {count} 个",
    "portfolio.noRefreshTitle": "本次会话尚未刷新价格",
    "portfolio.noRefreshBody": "后端也会根据配置的行情 provider 定时刷新。",
    "portfolio.positions": "持仓",
    "portfolio.tableSymbol": "代码",
    "portfolio.tableName": "名称",
    "portfolio.tableQty": "数量",
    "portfolio.tableAvgCost": "平均成本",
    "portfolio.tableLastPrice": "最新价",
    "portfolio.tableMarketValue": "市值",
    "portfolio.tablePl": "盈亏",
    "portfolio.tableWeight": "权重",
    "portfolio.tableStatus": "状态",
    "portfolio.noPositionsTitle": "还没有跟踪持仓",
    "portfolio.noPositionsBody": "导入持仓，把备忘录、决策和组合暴露连接起来。",
    "portfolio.mapSymbol": "代码",
    "portfolio.mapName": "名称",
    "portfolio.mapQuantity": "数量",
    "portfolio.mapAverageCost": "平均成本",
    "portfolio.mapCurrency": "币种",
    "portfolio.mapAccount": "账户",
    "portfolio.mapMarket": "市场",
    "portfolio.mapSector": "行业",
    "portfolio.mapImportedMarketValue": "市值",
    "portfolio.mapNotes": "备注",
    "portfolio.importTools": "导入工具",
    "portfolio.importToolsBody": "将文件或截图先转成同一张可编辑草稿表，确认后再合并写入持仓。",
    "portfolio.applyMapping": "应用映射",
    "portfolio.preparingDraft": "正在准备导入草稿...",
    "portfolio.clearDraft": "清除草稿",
    "portfolio.commitDraft": "确认草稿",
    "portfolio.fixDraftErrors": "请先修正或删除有阻断错误的行。",
    "portfolio.emptyDraftTitle": "暂无草稿行",
    "portfolio.emptyDraftBody": "选择 CSV、Excel 或截图，生成可编辑持仓草稿。",
    "portfolio.rowIssues": "问题",
    "portfolio.actions": "操作",
    "portfolio.cnyTotal": "人民币总额",
    "portfolio.cnyPl": "人民币盈亏",
    "portfolio.marketGroups": "个市场分组",
    "portfolio.fxStaleCount": "{count} 个汇率待刷新",
    "portfolio.marketAllocation": "市场配置",
    "portfolio.fxStale": "汇率待刷新",
    "portfolio.editPosition": "编辑持仓",
    "portfolio.savePosition": "保存持仓",

    "decisionDelta.eyebrow": "决策差异时间线",
    "decisionDelta.title": "实际决策 vs 反向持仓基线",
    "decisionDelta.refresh": "刷新差异",
    "decisionDelta.refreshing": "正在刷新...",
    "decisionDelta.visibleDelta": "可见差异",
    "decisionDelta.decisionCount": "决策数",
    "decisionDelta.staleSnapshots": "待刷新快照",
    "decisionDelta.lastRefresh": "最近刷新",
    "decisionDelta.never": "从未",
    "decisionDelta.sumDetail": "{positive} 个正差异 / {negative} 个负差异",
    "decisionDelta.impactDetail": "组合影响 {value}",
    "decisionDelta.filters": "筛选",
    "decisionDelta.symbol": "代码",
    "decisionDelta.action": "动作",
    "decisionDelta.delta": "差异",
    "decisionDelta.stale": "新鲜度",
    "decisionDelta.review": "复盘",
    "decisionDelta.sort": "排序",
    "decisionDelta.allSymbols": "全部代码",
    "decisionDelta.allActions": "全部动作",
    "decisionDelta.allDeltas": "全部差异",
    "decisionDelta.positive": "正差异",
    "decisionDelta.negative": "负差异",
    "decisionDelta.noSnapshot": "无快照",
    "decisionDelta.allFreshness": "全部新鲜度",
    "decisionDelta.staleOnly": "只看待刷新",
    "decisionDelta.freshOnly": "只看已更新",
    "decisionDelta.allReviews": "全部复盘",
    "decisionDelta.reviewedOnly": "只看已复盘",
    "decisionDelta.unreviewedOnly": "只看未复盘",
    "decisionDelta.sortDate": "最新决策",
    "decisionDelta.sortAbsoluteDelta": "差异最大",
    "decisionDelta.sortImpact": "影响最大",
    "decisionDelta.sortStale": "待刷新优先",
    "decisionDelta.timeline": "时间线",
    "decisionDelta.quantified": "已量化",
    "decisionDelta.unquantified": "待补基线",
    "decisionDelta.reviewed": "已复盘",
    "decisionDelta.notReviewed": "未复盘",
    "decisionDelta.noTimelineTitle": "没有匹配的决策",
    "decisionDelta.noTimelineBody": "调整筛选，或记录一条已量化决策。",
    "decisionDelta.noSelectionTitle": "尚未选择决策",
    "decisionDelta.noSelectionBody": "选择一条决策，查看实际腿和基线腿。",
    "decisionDelta.detail": "决策详情",
    "decisionDelta.rationale": "决策理由",
    "decisionDelta.expectedOutcome": "预期结果",
    "decisionDelta.actual": "实际",
    "decisionDelta.baseline": "基线",
    "decisionDelta.vs": "对比",
    "decisionDelta.assetVsCash": "持仓 vs 现金",
    "decisionDelta.cashVsHolding": "现金 vs 继续持有",
    "decisionDelta.cashVsHypothetical": "保留现金 vs 假设买入",
    "decisionDelta.manualComparison": "实际 vs 基线",
    "decisionDelta.latestSnapshot": "最新快照",
    "decisionDelta.actualValue": "实际价值",
    "decisionDelta.baselineValue": "基线价值",
    "decisionDelta.deltaPct": "差异百分比",
    "decisionDelta.snapshotHistory": "快照历史",
    "decisionDelta.snapshotHistoryLimited": "快照历史 · 最近 {count} 条",
    "decisionDelta.tableDate": "日期",
    "decisionDelta.tableActual": "实际",
    "decisionDelta.tableBaseline": "基线",
    "decisionDelta.tableDelta": "差异",
    "decisionDelta.tableStatus": "状态",
    "decisionDelta.fresh": "已更新",
    "decisionDelta.reviewNotes": "复盘备注",
    "decisionDelta.thesisEvidence": "支持证据",
    "decisionDelta.disconfirmingEvidence": "反证证据",
    "decisionDelta.lessons": "经验教训",
    "decisionDelta.candidatePrinciples": "候选原则",
    "decisionDelta.candidateChecklist": "候选检查项",
    "decisionDelta.saveReview": "保存复盘",
    "decisionDelta.savingReview": "正在保存...",
    "decisionDelta.adoptSelected": "采纳所选",
    "decisionDelta.adopting": "正在采纳...",
    "decisionDelta.saved": "复盘已保存",
    "decisionDelta.adopted": "候选已采纳",
    "decisionDelta.noCandidates": "暂无候选",
    "decisionDelta.refreshResult": "已刷新 {refreshed} 个，失败 {failed} 个",

    "memos.eyebrow": "备忘录",
    "memos.title": "Thesis、风险、催化剂和退出条件",
    "memos.newMemo": "新建备忘录",
    "memos.fieldTitle": "标题",
    "memos.titlePlaceholder": "公司或投资想法",
    "memos.fieldSymbol": "代码",
    "memos.fieldNotes": "原始笔记",
    "memos.notesPlaceholder": "粘贴观察、数据、链接、问题或粗略 thesis。",
    "memos.fieldTags": "标签",
    "memos.tagsPlaceholder": "质量, 复利",
    "memos.create": "创建备忘录",
    "memos.library": "备忘录库",
    "memos.noMemosTitle": "还没有备忘录",
    "memos.noMemosBody": "从粗略笔记开始，AI provider 可以提炼出结构化备忘录。",
    "memos.noSymbol": "无代码",
    "memos.aiExtract": "AI 提炼",
    "memos.notes": "笔记",
    "memos.noNotes": "暂无笔记",
    "memos.thesis": "Thesis",
    "memos.risks": "风险",
    "memos.catalysts": "催化剂",
    "memos.disconfirmingEvidence": "反证条件",
    "memos.aiChecklist": "AI 检查清单",

    "research.eyebrow": "研究库",
    "research.title": "研究记录、股票快照与候选沉淀",
    "research.distill": "提炼资料",
    "research.stockSnapshot": "股票快照",
    "research.portfolioReview": "组合复盘",
    "research.records": "研究记录",
    "research.filters": "筛选",
    "research.kindAll": "全部类型",
    "research.kindDistillation": "资料提炼",
    "research.kindStockSnapshot": "股票快照",
    "research.kindPortfolioReview": "组合复盘",
    "research.search": "搜索",
    "research.symbol": "代码",
    "research.sourceType": "资料类型",
    "research.sourceTitle": "资料标题",
    "research.sourceAuthor": "作者",
    "research.sourceContent": "资料内容",
    "research.recordTitle": "标题",
    "research.run": "运行",
    "research.summary": "摘要",
    "research.insights": "洞察",
    "research.risks": "风险",
    "research.checklist": "检查清单",
    "research.candidatePrinciples": "候选原则",
    "research.candidateChecklist": "候选检查项",
    "research.adoptSelected": "采纳所选",
    "research.loading": "加载中",
    "research.loadingRecords": "正在加载研究记录。",
    "research.loadingSelection": "正在加载所选研究记录。",
    "research.errorTitle": "研究请求失败",
    "research.noRecordsTitle": "暂无研究记录",
    "research.noRecordsBody": "运行资料提炼、股票快照或组合复盘后，会沉淀到研究库。",
    "research.noSelectionTitle": "尚未选择研究",
    "research.noSelectionBody": "选择一条记录，查看摘要、风险、检查清单和可采纳候选项。",

    "system.eyebrow": "投资体系",
    "system.title": "原则、清单、能力圈、决策规则",
    "system.aiRefine": "AI 整理",
    "system.principles": "原则",
    "system.checklist": "检查清单",
    "system.circle": "能力圈",
    "system.decisionRules": "决策规则",
    "system.save": "保存体系",
    "system.aiRefinement": "AI 整理结果",

    "profile.eyebrow": "投资画像",
    "profile.title": "从投资流程沉淀 RPG-like 反馈",
    "profile.level": "等级",
    "profile.xp": "XP",
    "profile.nextLevel": "下一等级需要 {xp}",
    "profile.badges": "徽章",
    "profile.biasSignals": "偏差信号",
    "profile.attributes": "属性",
    "profile.noBadgesTitle": "还没有徽章",
    "profile.noBadgesBody": "创建备忘录、导入持仓、记录可复盘决策后会解锁反馈。",
    "profile.noSignalsTitle": "暂无活跃信号",
    "profile.noSignalsBody": "当前规则检查没有标记流程风险。",
    "profile.ruleEvents": "规则事件",

    "settings.eyebrow": "设置",
    "settings.title": "AI provider 与凭据配置",
    "settings.provider": "Provider",
    "settings.providerMock": "Mock",
    "settings.providerOpenai": "OpenAI-compatible",
    "settings.providerCli": "CLI provider",
    "settings.cliProvider": "CLI provider",
    "settings.cliProviderCodex": "Codex CLI device 登录",
    "settings.openaiBaseUrl": "OpenAI-compatible Base URL",
    "settings.openaiModel": "OpenAI-compatible 模型",
    "settings.openaiApiKey": "API Key",
    "settings.openaiApiKeyPlaceholder": "留空则保留当前 key",
    "settings.keyConfigured": "当前已配置 key。",
    "settings.keyMissing": "尚未配置 API key。",
    "settings.cliPath": "CLI 路径",
    "settings.cliModel": "CLI 模型覆盖",
    "settings.cliProfile": "CLI profile",
    "settings.cliLoginCommand": "CLI 登录命令",
    "settings.cliHelp": "如果使用 Codex，先在终端运行一次该命令，完成浏览器/device-code 登录，再选择 CLI provider。",
    "settings.persist": "写入 .env 持久化",
    "settings.save": "保存 AI 设置",
    "settings.saved": "设置已保存",
    "settings.envNote": "运行时设置会立即生效。写入 .env 后，后端重启仍会保留。",
    "settings.mockNote": "Mock 模式完全本地、确定性输出。"
  }
} as const;
